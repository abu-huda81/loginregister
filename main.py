...
def signup_user(
    ...
    signup = UserModel(
        email=email, username=username, password=get_password_hash(password)
    )
...
def verify_user(token, db: Session = Depends(sess_db)):
    ...
    if db_user.is_active == True:
        return "your account  has been allreay activated"
...
@app.delete("/user/{id}")
def delete_user(id: int, db: Session = Depends(sess_db)):
    userRepository = UserRepository(db)
